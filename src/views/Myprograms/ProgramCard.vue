...
import { EmployerProgram, studentClassroom,TeacherProgramData } from '../../store/Database/types/types'
...
export default class ProgramCard extends Vue {
  loader = false
    ...
    async onProgramChange(){
      this.loader = true
      const classroomData = await FbStore.findRelativeClassroom({employerProgramId:this.program.employerProgramId,studentId:FbStore.FBUser!.uid})
      const studentClassroomData = (await FbStore.firestore.collection("studentClassroom").doc(classroomData.classroomId+FbStore.FBUser!.uid).get()).data<studentClassroom>()
      const project = await FbStore.findRelativeProject({classroomId:classroomData.classroomId, studentId:FbStore.FBUser!.uid})
      const keysOfSequence = {programBrief:studentClassroomData?.finishedIntrovideo,
...
let progress = [...Object.values(keysOfSequence)].reduce((total,current)=> {
