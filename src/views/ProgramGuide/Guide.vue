<template>
  <v-row class="ml-0 mr-0">
    <v-col cols="2" class="pl-12 pr-12 guide__bar">
      <!-- MAIN GROUP -->
      <v-row class="guide__mainrow">

        <span class="guide__bigdot guide__activebigdot guide__background_blue"><i class="guide__icon_white fas fa-chalkboard-teacher guide__programicon"></i></span>

        <span class="guide__maintext guide__activemaintext">Externship Day</span>
      </v-row>





      <!-- SUB GROUP -->
      <v-row class="guide__bridge"></v-row>
      <v-row class="guide__subrow">
        <span class="guide__smalldot"></span>

        <span class="guide__smalldotfilled guide__background_blue"></span>

        <span class="guide__subtext">Agenda</span>
      </v-row>








            <!-- SUB GROUP -->
      <v-row class="guide__bridge"></v-row>
      <v-row class="guide__subrow">
        <span class="guide__smalldot"></span>
        <span class="guide__smalldotfilled guide__background_blue"></span>
        <span class="guide__subtext">Brief</span>
      </v-row>

                  <!-- SUB GROUP -->
      <v-row class="guide__bridge"></v-row>
      <v-row class="guide__subrow">
        <span class="guide__smalldot"></span>
        <span class="guide__smalldotfilled guide__background_blue"></span>
        <span class="guide__subtext">Manage</span>
      </v-row>

      <!-- MAIN GROUP -->
      <v-row class="guide__bridge"></v-row>
      <v-row class="guide__mainrow">
        <span class="guide__bigdot  guide__background_green guide__activebigdot"><i class="fas fa-project-diagram guide__icon_white guide__programicon"></i></span>
        <span class="guide__maintext guide__activemaintext ">Project</span>
      </v-row>
      <!-- SUB GROUP -->
      <v-row class="guide__bridge"></v-row>
      <v-row class="guide__subrow">
        <span class="guide__smalldot"></span>
        <span class="guide__smalldotfilled guide__background_green"></span>
        <span class="guide__subtext">Intro Video</span>
      </v-row>
      <!-- SUB GROUP -->
      <v-row class="guide__bridge"></v-row>
      <v-row class="guide__subrow">
        <span class="guide__smalldot"></span>
        <span class="guide__smalldotfilled guide__background_green"></span>
        <span class="guide__subtext">Train</span>
      </v-row>
      <!-- SUB GROUP -->
      <v-row class="guide__bridge"></v-row>
      <v-row class="guide__subrow">
        <span class="guide__smalldot"></span>
        <span class="guide__smalldotfilled guide__background_green"></span>
        <span class="guide__subtext">Practice</span>
      </v-row>

            <!-- SUB GROUP -->
      <v-row class="guide__bridge"></v-row>
      <v-row class="guide__subrow">
        <span class="guide__smalldot"></span>
        <span class="guide__smalldotfilled guide__background_green"></span>
        <span class="guide__subtext">Research</span>
      </v-row>

            <!-- SUB GROUP -->
      <v-row class="guide__bridge"></v-row>
      <v-row class="guide__subrow">
        <span class="guide__smalldot"></span>
        <span class="guide__smalldotfilled guide__background_green"></span>
        <span class="guide__subtext">Ideate</span>
      </v-row>

            <!-- SUB GROUP -->
      <v-row class="guide__bridge"></v-row>
      <v-row class="guide__subrow">
        <span class="guide__smalldot"></span>
        <span class="guide__smalldotfilled guide__background_green"></span>
        <span class="guide__subtext">Prepare</span>
      </v-row>

                  <!-- SUB GROUP -->
      <v-row class="guide__bridge"></v-row>
      <v-row class="guide__subrow">
        <span class="guide__smalldot"></span>
        <span class="guide__smalldotunfilled"></span>
        <span class="guide__subtext">Hack Day</span>
      </v-row>

                        <!-- SUB GROUP -->
      <v-row class="guide__bridge"></v-row>
      <v-row class="guide__subrow">
        <span class="guide__smalldot"></span>
        <span class="guide__smalldotunfilled"></span>
        <span class="guide__subtext">Reflection</span>
      </v-row>

                              <!-- SUB GROUP -->
      <v-row class="guide__bridge"></v-row>
      <v-row class="guide__subrow">
        <span class="guide__smalldot"></span>
        <span class="guide__smalldotunfilled"></span>
        <span class="guide__subtext">Design & Prototype</span>
      </v-row>

                                    <!-- SUB GROUP -->
      <v-row class="guide__bridge"></v-row>
      <v-row class="guide__subrow">
        <span class="guide__smalldot"></span>
        <span class="guide__smalldotunfilled"></span>
        <span class="guide__subtext">Package</span>
      </v-row>

                                    <!-- SUB GROUP -->
      <v-row class="guide__bridge"></v-row>
      <v-row class="guide__subrow">
        <span class="guide__smalldot"></span>
        <span class="guide__smalldotunfilled"></span>
        <span class="guide__subtext">Demo Day</span>
      </v-row>

                                          <!-- SUB GROUP -->
      <v-row class="guide__bridge"></v-row>
      <v-row class="guide__subrow">
        <span class="guide__smalldot"></span>
        <span class="guide__smalldotunfilled"></span>
        <span class="guide__subtext">Exit Form</span>
      </v-row>



    <!-- MAIN GROUP -->
      <v-row class="guide__bridge"></v-row>
      <v-row class="guide__mainrow">
        <span class="guide__bigdot"><i class="fas fa-seedling guide__programicon guide__icon_gray"></i></span>
        <span class="guide__maintext">Internship</span>
      </v-row>

      <!-- SUB GROUP -->
      <v-row class="guide__bridge"></v-row>
      <v-row class="guide__subrow">
        <span class="guide__smalldot"></span>
        <span class="guide__smalldotunfilled"></span>
        <span class="guide__subtext">Interview Offer</span>
      </v-row>

      <!-- SUB GROUP -->
      <v-row class="guide__bridge"></v-row>
      <v-row class="guide__subrow">
        <span class="guide__smalldot"></span>
        <span class="guide__smalldotunfilled"></span>
        <span class="guide__subtext">First Interview</span>
      </v-row>
      <!-- SUB GROUP -->
      <v-row class="guide__bridge"></v-row>
      <v-row class="guide__subrow">
        <span class="guide__smalldot"></span>
        <span class="guide__smalldotunfilled"></span>
        <span class="guide__subtext">Final Interview</span>
      </v-row>
      <!-- SUB GROUP -->
      <v-row class="guide__bridge"></v-row>
      <v-row class="guide__subrow">
        <span class="guide__smalldot"></span>
        <span class="guide__smalldotunfilled"></span>
        <span class="guide__subtext">Position Offer</span>
      </v-row>

      <!-- SUB GROUP -->
      <v-row class="guide__bridge"></v-row>
      <v-row class="guide__subrow">
        <span class="guide__smalldot"></span>
        <span class="guide__smalldotunfilled"></span>
        <span class="guide__subtext">Next Program</span>
      </v-row>








    </v-col>
  </v-row>
</template>





<script lang="ts">
import Vue from "vue";
import Component from "vue-class-component";
@Component
export default class guide extends Vue {}
</script>